...
class Splash {
    ...
    async startAnimation() {
        let splashes = [
            { "message": "Locura total!!!", "author": "EonSafio" },
            { "message": "El Fuego se Expande!", "author": "EonSafio" },
            { "message": "Reviil", "author": "EonSafio" }
        ]
    ...
    async checkUpdate() {
        ...
        ipcRenderer.invoke('update-app').then(err => {
            if (err.error) {
                ...
                this.shutdown(`error al buscar actualización :<br>${error}`);
            }
        })
    ...
    shutdown(text) {
        this.setStatus(`${text}<br>Se Detiene en 5s`);
        let i = 4;
        setInterval(() => {
